...
//  Skeleton
//
//  Created by Lucas Nascimento on 05/07/19.
...
public class ABLoader {
    ...
    public func startSmartShining(_ views: [UIView]) {
        //addLoader(view, start: true)
        for subView in views {
            subView.layoutIfNeeded()
            animate(view: subView, start: true)
        }
    }
    ...
    private func animate(view: UIView, start: Bool) {
        if start {
            ...
            if let smartLayers = view.layer.sublayers?.filter({
                $0.name == "colorLayer" || $0.name == "loaderLayer"
